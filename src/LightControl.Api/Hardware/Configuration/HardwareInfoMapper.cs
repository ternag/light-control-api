...
public class HardwareInfoMapper : IHardwareInfoMapper
{
    ...
    public Dictionary<LedId, Pin> GetPins(IHardwareInfo hardwareInfo)
    {
        ...
        foreach (var device in hardwareInfo.Devices)
            try
            {
                var concreteDevice = _hardwareDeviceFactory.Create(device);

                foreach (var mapInfo in device.Map) pins.Add(mapInfo.Id, new Pin(mapInfo.Pin, concreteDevice));
            }
            catch (Exception e)
            {
